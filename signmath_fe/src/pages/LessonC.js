...
export const LessonC = () => {
    const [items, setItems] = useState([])
    const navigate = useNavigate();

    useEffect(() => {
        api.filter('lessons/filter','c')
            .then(res => {
        ...
        >
            <div className={styles.home}  style={{ color: "#973333" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-Start", margin: '5px' }}>
                    <img src={back} alt="back" width="5%" onClick={() => navigate('/npc')} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom:'10px'}}>
                    <h1>මූලික වර්ණ හදුනා ගැනීම</h1>
                </div>
                <Row className="mt-1">
                    <Col sm={4}>
                    ...
                    <Col>
                        ...
                        <Link to="/qc" style={{ textDecoration: 'none' }}>
