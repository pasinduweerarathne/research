...
db = firestore.client()
ref = db.collection('lessons')

lessonsAPI = Blueprint('lessonsAPI', __name__)

@lessonsAPI.route('', methods=['POST'])
def create_record():
    try:
        _id = uuid.uuid4()
...
@lessonsAPI.route('/filter', methods=['GET'])
# @jwt_required
def filter_records():
    data = []
    try:
...
@lessonsAPI.route('/<id>', methods=['GET'])
def get_item(id):
